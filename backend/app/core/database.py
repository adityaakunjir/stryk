...
async def create_db_tables():
    ...
    async with engine.begin() as conn:
        ...
        try:
            ...
            await conn.execute(text('ALTER TABLE users ADD COLUMN IF NOT EXISTS "progressionPoints" INTEGER NOT NULL DEFAULT 0;'))
            await conn.execute(text('ALTER TABLE users ADD COLUMN IF NOT EXISTS "statCap" INTEGER NOT NULL DEFAULT 75;'))
            await conn.execute(text('ALTER TABLE users ADD COLUMN IF NOT EXISTS "verifiedMatchCount" INTEGER NOT NULL DEFAULT 0;'))
            await conn.execute(text('ALTER TABLE users ADD COLUMN IF NOT EXISTS "totalPointsEarned" INTEGER NOT NULL DEFAULT 0;'))
            ...
            try:
                ...
                await conn.execute(text('CREATE INDEX IF NOT EXISTS ix_users_xp ON users (xp);'))
                await conn.execute(text('CREATE INDEX IF NOT EXISTS ix_users_goals ON users (goals);'))
                await conn.execute(text('CREATE INDEX IF NOT EXISTS ix_users_assists ON users (assists);'))
                ...
                await conn.execute(text('CREATE INDEX IF NOT EXISTS ix_match_stats_match_user_status ON match_stats ("matchId", "userId", status);'))
                await conn.execute(text('CREATE INDEX IF NOT EXISTS ix_matches_completedAt_format ON matches ("completedAt", format);'))
