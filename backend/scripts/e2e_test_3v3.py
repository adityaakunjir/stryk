...
async def run():
    async with httpx.AsyncClient(timeout=30.0) as c:
        ...
        # Step 10.5: Backdate verification deadline so /complete isn't blocked
        # (In production this fires after 24h; in tests we fast-forward the DB)
        print("\n== Step 10.5: Backdate deadlines so /complete can run immediately ==")
        import sqlite3 as _sqlite3
        _con = _sqlite3.connect("stryk.db")
        _con.execute(
            "UPDATE matches SET verificationDeadline=datetime('now','-1 hour'), submissionDeadline=datetime('now','-1 hour') WHERE id=?",
            (match_id,)
        )
        _con.commit()
        _con.close()
        print("   OK   Deadlines backdated")
